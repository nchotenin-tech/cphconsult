...
The first live run passed eight checks but failed the final same-connection assertion. The permission probes used pool.query, whose error path evicts the client. A regression test using the installed pg.Pool with a deterministic transport stub reproduced the PID change (2 instead of 1). Expected denials now run through withActorTransaction, rolling back before returning the connection. The same-PID assertion is preserved. This fixes the test harness; it does not weaken database permissions. A fresh live run is still required to confirm all nine checks.
